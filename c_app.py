...
ax.plot(city_counts.index, city_counts.values, marker='o', color='teal', linewidth=2)
ax.set_title("Crimes Per City", fontsize=8)
...
fig, ax = plt.subplots(figsize=(3,2.5))
ax.plot(crime_counts.index, crime_counts.values, marker='o', color='orange', linewidth=2)
ax.set_title("Crime Type Distribution", fontsize=8)
plt.xticks(rotation=45, fontsize=6)
plt.yticks(fontsize=6)
plt.tight_layout()
st.pyplot(fig)
...
fig, ax = plt.subplots(figsize=(3,2.5))
df_gender.plot(kind='bar', ax=ax, color=['#FF6F61', '#6B5B95'], width=0.6, edgecolor='black')
ax.set_title("Victim vs Suspect Gender Distribution", fontsize=8)
...
if 'Date' in df.columns:
    ...
    fig, ax = plt.subplots(figsize=(3,2.5))
    ax.plot(df_time_series.index, df_time_series.values, marker='o', color='purple', linewidth=2)
    ax.fill_between(df_time_series.index, df_time_series.values, alpha=0.2, color='purple')
    ax.set_title("Crime Counts Over Time", fontsize=8)
    ax.set_xlabel("Date", fontsize=6)
    ax.set_ylabel("Number of Crimes", fontsize=6)
    plt.xticks(rotation=45, fontsize=6)
    plt.yticks(fontsize=6)
    plt.tight_layout()
    st.pyplot(fig)
