...
for i, city_name in enumerate(city_counts.index):
    ax.scatter(city_name, city_counts[city_name], color=line_colors[i % len(line_colors)], s=80, label=city_name)
ax.plot(city_counts.index, city_counts.values, color='grey', linewidth=1.5, alpha=0.5)
...
for i, crime_name in enumerate(crime_counts.index):
    ax.scatter(crime_name, crime_counts[crime_name], color=line_colors[i % len(line_colors)], s=80, label=crime_name)
ax.plot(crime_counts.index, crime_counts.values, color='grey', linewidth=1.5, alpha=0.5)
...
ax.set_title("Victim vs Suspect Gender", fontsize=14)
